...
public class AppIntercepter extends HandlerInterceptorAdapter {
	 ...
	 * boolean 값을 전달하는데 false 인 경우 controller를 실행 시키지 않고 요청을 종료한다. 보통 이곳에서 각종
	 * 체크작업과 로그를 기록하는 작업을 진행한다.
	 */
	...
	/**
	 * Servlet 3.0부터 비동기 요청이 가능해짐에 따라 비동기 요청 시 PostHandle와 afterCompletion메서드를
	 * 수행하지 않고 이 메서드를 수행하게 된다. 거의 사용 안함.
	 */
	@Override
